...
import bowserImage from "./Bowser.jpg"; // Ensure the correct image path
...
const Browserdetails = () => {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        ...
        console.log("History API response:", result); // Debug API response
        if (response.ok) {
          // Handle both paginated response (result.data) and direct array response
  ...
  const validateRow = (row, index) => {
    ...
    if (!row.browserNo) rowErrors.browserNo = "Browser No. is required";
    if (!row.product) rowErrors.product = "Product is required";
    if (!row.quantity || row.quantity <= 0) rowErrors.quantity = "Valid quantity is required";
  ...
  const handleInputChange = (index, field, value) => {
    ...
    setRows(updatedRows);
      ...
      if (field === "quantity") {
    updatedRows[index][field] = value === "" ? "" : parseInt(value, 10);
  } else {
    updatedRows[index][field] = value;
  }
  setRows(updatedRows);
  ...
  const toggleCheck = (index, field) => {
    const updatedRows = [...rows];
    updatedRows[index][field] = updatedRows[index][field] === "Pending" ? "Checked" : "Pending";
    setRows(updatedRows);
  };
  ...
  const addNewRow = () => {
    ...
    const rowErrors = validateRow(newRow, rows.length);
    if (Object.keys(rowErrors).length === 0 || window.confirm("Some fields are empty. Add new row anyway?")) {
      setRows([...rows, newRow]);
  ...
  const handleSubmit = async () => {
    // Validate all rows
    const allErrors = rows.map((row, index) => validateRow(row, index));
    setErrors(allErrors);
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log("History refresh API response:", historyResult); // Debug API response
        if (historyResponse.ok) {
          // Handle both paginated response (result.data) and direct array response
          setHistory(Array.isArray(historyResult.data) ? historyResult.data : Array.isArray(historyResult) ? historyResult : []);
        } else {
          setSubmitStatus({ type: "error", message: historyResult.message || "Error fetching history after submit." });
        }
        // Reset rows
        setRows([
          {
  ...
  const toggleHistory = () => {
    console.log("Toggling history, current showHistory:", showHistory); // Debug state change
