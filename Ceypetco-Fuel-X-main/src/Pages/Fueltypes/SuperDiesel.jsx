...
const PRODUCT_NAME = "Lanka Super Diesel";
...
function productMatches(name, wanted = PRODUCT_NAME) {
  ...
  return a === b || a.includes("super diesel");
}
// -----------------------------
...
const SuperDiesel = () => {
  ...
  // ---- Tank levels (analytics first; fallback to scanned-text) ----
  useEffect(() => {
    (async () => {
      try {
        setLoading(true);
        ...
        const s =
          ...
          series.find((s) => normalize(s.fuelType).includes("super diesel"));
        ...
        // 2) fallback to scanned-text if missing/NULL
        ...
        // Optional: prefill pump meter with today's level if present
        if (typeof tVal === "number") setTodayReading(String(tVal));
      } catch (e) {
        setErr("Could not load tank levels. Check the endpoint and fuelTypes encoding.");
      } finally {
        setLoading(false);
      }
    })();
